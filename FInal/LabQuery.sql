...
Group by ProductName;
	...
	/*Q2 Query*/
select ProductName, count(ProductName) as numRating
...
order by count(*);
	...
	/*Q3 Query*/
	/* DATEDIFF(timeAccuracy,endDate,StartDate)*/
	
	
select ProductName, AVG(DATEDIFF(day,EndDate,DeliveryDate)) as average
from Orders,ProductsInOrders
where month(EndDate)='6' AND year(EndDate) ='2018'AND Orders.OrdersID = ProductsInOrders.OrderId AND OrderStatus='Delivered'
group by ProductName; 

	/*Q4 Query*/
select Name,AVG(DATEDIFF(day,FillingDateTime,AddressedDateTime)) as Latency
from Complaint, Employee
where Complaint.EmployeeID = Employee.ID and Status = 'Handled'
